...
create_kube_cluster () {
    ...
    if [ -f "$BASE_DIR/configs/k3d/default.yaml" ]; then
        k3d cluster create --config $BASE_DIR/configs/k3d/default.yaml --wait
    else
        printf "\nCould not find %s, exiting....\n" "$BASE_DIR/configs/k3d/default.yaml"
        exit 1
    fi
