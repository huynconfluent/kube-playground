...
eval $BASE_DIR/scripts/creds/create-sasl-oauth.sh -m "$NAMESPACE" -u "$BASE_DIR/configs/creds/default-oauth-users.json" -m "/mnt/sslcerts/keypair/mds-keypair-public.pem" -t "/mnt/sslcerts/SECRET_NAME/truststore.p12" -p "topsecret" -c
# call SASL/PLAIN
...
eval $BASE_DIR/scripts/creds/create-basic-auth.sh -p "schemaregistry" -u "schemaregistry:schemaregistry-secret:admin,srconsumer:srconsumer-secret:user,srproducer:srproducer-secret:user,sradmin:sradmin-secret:admin,srexporter:srexporter-secret:developer,superuser:superuser-secret:admin" -n "$NAMESPACE" -c 
...
eval $BASE_DIR/scripts/creds/create-basic-auth.sh -p "connect" -u "connect:connect-secret,connectconsumer:connectconsumer-secret,connectproducer:connectproducer-secret,connectadmin:connectadmin-secret,superuser:superuser-secret" -n "$NAMESPACE" -c 
eval $BASE_DIR/scripts/creds/create-basic-auth.sh -p "prometheus" -u "controlcenter:controlcenter-secret,superuser:superuser-secret" -n "$NAMESPACE" -c
eval $BASE_DIR/scripts/creds/create-basic-auth.sh -p "alertmanager" -u "controlcenter:controlcenter-secret,superuser:superuser-secret" -n "$NAMESPACE" -c
# call SASL/DIGEST
